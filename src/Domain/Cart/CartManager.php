...
class CartManager
{
    ...
    public function add(Product $product, int $quantity = 1, $optionValues = []): Model|Builder
    {
        $cart = Cart::query()
            ->updateOrCreate([
        ...
        $cartItem = $cart->cartItems()->updateOrCreate(
            [
                ...
                'string_option_values' => $this->stringedOptionValues()
            ],
            [
                'price' => $product->price,
    ...
    public function quantity(CartItem $item, int $quantity): void
    {
        $this->forgetCache();
...
//            });
    ...
    public function items(): Collection
    {
        ...
        return CartItem::query()->with(['product', 'optionValues.option'])
    ...
    {
        $cart = Cart::query()->where('storage_id', $old)
